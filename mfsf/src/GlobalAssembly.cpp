    ...
    std::vector<double> &invJac_all_qp)
{
    ...
    for (int jj = 0; jj < nlocalelemy; ++jj)
    {
        for (int ii = 0; ii < nlocalelemx; ++ii)
        {
            ...
            jacobian_all_qp.insert(jacobian_all_qp.end(),
                locassem->GetJacobianQP().begin(), locassem->GetJacobianQP().end());
            invJac_all_qp.insert(invJac_all_qp.end(),
                locassem->GetInvJacQP().begin(), locassem->GetInvJacQP().end());
