...
std::vector<int> IENGenerator::GenerateIEN3D(
    const int &nElemX, const int &nElemY, const int &nElemZ)
{
    std::vector<int> IEN{};
    const int nFuncX = nElemX + 1;
    const int nFuncY = nElemY + 1;
    const int nFuncZ = nElemZ + 1;
    ...
    for (int k = 0; k < nElemZ; ++k)
    {
        for (int j = 0; j < nElemY; ++j)
        {
            for (int i = 0; i < nElemX; ++i)
            {
                IEN.push_back(k * nFuncX * nFuncY + j * nFuncX + i);
                IEN.push_back(k * nFuncX * nFuncY + j * nFuncX + i + 1);
                IEN.push_back(k * nFuncX * nFuncY + (j + 1) * nFuncX + i);
                IEN.push_back(k * nFuncX * nFuncY + (j + 1) * nFuncX + i + 1);

                IEN.push_back((k + 1) * nFuncX * nFuncY + j * nFuncX + i);
                IEN.push_back((k + 1) * nFuncX * nFuncY + j * nFuncX + i + 1);
                IEN.push_back((k + 1) * nFuncX * nFuncY + (j + 1) * nFuncX + i);
                IEN.push_back((k + 1) * nFuncX * nFuncY + (j + 1) * nFuncX + i + 1);
            }
        }
    }
    ...
    const BSplineBasis * const &basis3)
{
    ...
    const int nElem = (m - p) * (n - q) * (o - r);
    const int nLocBas = (p + 1) * (q + 1) * (r + 1);
    ...
    for (int k = 0; k < o; ++k)
    {
        for (int j = 0; j < n; ++j)
        {
            for (int i = 0; i < m; ++i)
            {
                ...
                if (i >= p && j >= q && k >= r)
                {
                    for (int kloc = r; kloc >= 0; --kloc)
                    {
                        for (int jloc = q; jloc >= 0; --jloc)
                        {
                            for (int iloc = p; iloc >= 0; --iloc)
                            {
                                int B = A - iloc - jloc * m - kloc * m * n - 1;
                                IEN.push_back(B);
                            }
                        }
                    }
